...
public class EnemyStandardMovements_BAK : MonoBehaviour {
	...
	private float speedRatio = 0.011f;
	...
	void FixedUpdate(){
		...
		Debug.DrawLine(m_rigidbody.position, agent.destination, Color.red, 1f);
	...
	void Update () {
        ...
        if (isChasing)
        {
			...
			if (isTurningAround) {
				...
				m_rigidbody.AddRelativeForce (transform.right * Time.deltaTime * 80 * (1 - speedRatio) * turnDirection);
				...
				if (agent.remainingDistance < distanceMin) {
					...
					m_rigidbody.AddForce ((transform.position - hero.position) * 80 * escapeSpeedRatio);
		...
		m_rigidbody = GetComponent<Rigidbody>();
		m_rigidbody.AddRelativeForce(transform.forward * Time.deltaTime * 80 * speed * speedRatio);
		

        }
		
		agent.velocity = m_rigidbody.velocity;
